...
// The setup component and the CUDA implementation are a single ABI unit.
// risc0-groth16-sys 0.1.0 generated its assets with sppark 0.1.12, whose
// BN254 scalar has 16-byte alignment. Keep the exact upstream record layout
// and fail the build if a transitive dependency silently changes it.
struct coeff_t {
  uint32_t m, c, s;
  fr_t value;
};
static_assert(sizeof(fr_t) == 32, "unexpected sppark BN254 scalar size");
static_assert(sizeof(coeff_t) == 48, "unexpected Groth16 coefficient ABI");
...
public:
  ...
  preprocessed_coeffs(const gpu_t& gpu, const char* file_path) : gpu(gpu) {
    ...
    struct stat st;
    if (fstat(pcoeffs_file, &st) != 0) {
      int err = errno;
      close(pcoeffs_file);
      throw sppark_error{err, "fstat(\"%s\") failed: ", file_path};
    }
    ...
    try {
      ...
      const size_t coeff_count = d_coeffs_a.size() + d_coeffs_b.size();
      const size_t index_count = d_ind_idx_a.size() + d_ind_idx_b.size();
      const size_t expected_file_size =
          4 * sizeof(size_t) + coeff_count * sizeof(coeff_t) +
          index_count * sizeof(uint32_t);
      if (file_size != expected_file_size) {
        throw sppark_error{EINVAL,
                           "preprocessed coefficient asset has invalid length. expected %zu, got "
                           "%zu: ",
                           expected_file_size,
                           file_size};
      }
      ...
      gpu.HtoD(&d_coeffs[0], (coeff_t*)&ptr[0], coeff_count);

      ptr += coeff_count * sizeof(coeff_t);
      gpu.HtoD(
  ...
  void write_to_file(const char* file_path) {
    ...
    try {
      ...
      const size_t coeff_count = d_coeffs_a.size() + d_coeffs_b.size();
      gpu.DtoH((coeff_t*)file_ptr, &d_coeffs[0], coeff_count);

      file_ptr += coeff_count * sizeof(coeff_t);
      gpu.DtoH(
