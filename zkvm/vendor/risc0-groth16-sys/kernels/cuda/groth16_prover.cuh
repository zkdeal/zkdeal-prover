...
public:
  ...
  groth16_proof prove(const char* public_file_path, const fr_t* witness) {
    ...
    // HtoD is asynchronous on gpu's zero stream, while the A-polynomial
    // kernel runs on gpu[0]. Without an explicit dependency, that kernel can
    // observe a partially uploaded witness and produce an invalid Groth16
    // proof. Keep the streams concurrent, but make the first cross-stream
    // witness reader wait for the upload event.
    event_t witness_ready(gpu);
    ...
    try {
      ...
      CUDA_OK(cudaGetLastError());
      // The MSM implementation decomposes device-resident scalars on gpu[2].
      // d_witness is produced on gpu's zero stream, so the first MSM must not
      // read it until the fuzz-add kernel has completed.
      event_t fuzzed_witness_ready(gpu);
      fuzzed_witness_ready.wait(gpu[2]);
